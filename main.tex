...
A finite-state approximation to the finite-time transition probability distribution
$P(S(t)|S(0))$
...
There is a prefix list (or tree) of sequences.
Node 1 corresponds to the empty sequence.
Nodes $N>1$ correspond to nonempty sequences $S_N$.
Character $\alpha_N$ is the last character of $S_N$.
The parent node $\beta_N$ corresponds to the longest prefix of $S_N$:
removing $\alpha_N$ from $S_N$ leaves $S_{\beta_N}$.
...
\\
F(1,j,t) & = &
F(1,\beta_j,t) U_I(\alpha_j,t)
\\
F(i,1,t) & = &
F(\beta_i,1,t) U_D(\alpha_i,t)
\\
F(i,j,t) & = &
F(\beta_i,\beta_j,t) U_M(\alpha_i,\alpha_j,t)
+ F(\beta_i,j,t) U_I(\alpha_i,t)
+ F(i,\beta_j,t) U_D(\alpha_j,t)
\\
...
p(t) \end{bmatrix}
\\
U_M(\alpha_i,\alpha_j,t) & = &
\begin{bmatrix}
a(t) & 0 & 0 \\
f(t) & 0 & 0 \\
p(t) & 0 & 0 
\end{bmatrix}
\exp(Qt)_{\alpha_i \alpha_j}
\\
U_I(\alpha_j,t) & = &
\begin{bmatrix}
0 & b(t) & 0 \\
...
\begin{itemize}
    \item The list $A_M$ of pairs of aligned characters $(\alpha,\alpha')$ from ancestor $S(0)$ and descendant $S(t)$;
    \item The list $A_{ID}$ of pairs of unaligned gap sequences $(\gamma,\gamma')$ that were deleted and inserted in between.
\end{itemize}

The alignment probability, computable in time $O(L)$ (and in practice very fast), is
\[
P(A_M,A_{ID},S(t)|S(0)) =
\prod_{(\alpha,\alpha') \in A_M} \exp(Qt)_{\alpha \alpha'}
\prod_{(\gamma,\gamma') \in A_{ID}} G(|\gamma|,|\gamma'|,t)
\prod_{\alpha'\in \gamma'} \pi_{\alpha'}
\]
...
Both forms of the pairwise likelihood (alignment specified, or unspecified) can be extended to multiple sequences. A choice may be faced as to whether to sum out ancestral sequences (alignment specified) or ancestral alignments (alignment unspecified).
