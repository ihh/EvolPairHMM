...
Consider a version of the model where each element in the sequence can either be a point substitution site, or a nested sequence.
This generates a structure like a parse tree.

Suppose there are different types of evolutionary sequence.
In a sequence of type $i$, the evolutionary parameters are $(\lambda_i,\mu_i)$.
The probability that a random element in this sequence is another recursively nested subsequence is $\eta_i$.
If so, the probability that the nested subsequence is of type $j$ is $\tau_{ij}$.

If the element is not a subsequence, the probability that it is a point substitution site of type $k$ is $\sigma_{ik}$.
The rate matrix for such a site is $Q^{(k)}$ with stationary distribution $\pi^{(k)}$.

The equilibrium distribution is a stochastic context-free grammar (SCFG),
and the joint distribution over alignments is a Pair SCFG

\[
\begin{array}{rclll}
  M_i & \to & A_i & M_i & (\lambda_i/\mu_i) a_i \\
      & | & Y_i & I_i & b_i \\
      & | & X_i & D_i & (\lambda_i/\mu_i) c_i \\
      & | & \epsilon & & (1 - \lambda_i/\mu_i) (1-b_i) \\
  I_i & \to & A_i & M_i & (\lambda_i/\mu_i) f_i \\
      & | & Y_i & I_i & g_i \\
      & | & X_i & D_i & (\lambda_i/\mu_i) h_i \\
      & | & \epsilon & & (1 - \lambda_i/\mu_i) (1-g_i) \\
  D_i & \to & A_i & M_i & (\lambda_i/\mu_i) p_i \\
      & | & Y_i & I_i & q_i \\
      & | & X_i & D_i & (\lambda_i/\mu_i) r_i \\
      & | & \epsilon & & (1 - \lambda_i/\mu_i) (1-q_i) \\
  A_i & \to & \omega_k & \omega'_k & (1-\eta_i) \sigma_{ik} \pi^{(k)}_{\omega} \exp(Q^{(k)} t)_{\omega \omega'} \\
      & | & M_j & & \eta_i \tau_{ij} \\
  X_i & \to & \omega_k & & (1-\eta_i) \sigma_{ik} \pi^{(k)}_{\omega} \\
      & | & D^\ast_j & & \eta_i \tau_{ij} \\
  Y_i & \to & \omega'_k & & (1-\eta_i) \sigma_{ik} \pi^{(k)}_{\omega'} \\
      & | & I^\ast_j & & \eta_i \tau_{ij} \\
  D^\ast_i & \to & X_i & D^\ast_i & \lambda_i/\mu_i \\
        & | & \epsilon & & 1 - \lambda_i/\mu_i \\
  I^\ast_i & \to & Y_i & I^\ast_i & \lambda_i/\mu_i \\
        & | & \epsilon & & 1 - \lambda_i/\mu_i
\end{array}
\]
