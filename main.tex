...
With rate $\rho_i$, both links are resampled from their stationary distributions, conditional on their types (which remain fixed).
...
\end{array}$ & $\begin{array}{rclll}
  B_i & \to & L_j & L_k & \kappa_{ijk} \exp(-\rho_i t) \\
      & | & L^{xy}_j & L^{xy}_k & \kappa_{ijk} (1 - \exp(-\rho_i t)) \\
