...
for an alphabet of size $A$, neglecting the effect of substitutions,
...
An upper bound for $a$ is $\exp(-\mu t)$, so the maximum time $T$ to which we should simulate is $T = \lim_{n \to \infty} T_n$ where
\[
