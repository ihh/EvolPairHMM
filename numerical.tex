...
\begin{eqnarray*}
  ...
  J^\tonull_{ij} & = & \left\{ \begin{array}{ll} 1 & \mbox{if $j \in \sigma_N$} \\ 0 & \mbox{if $j \notin \sigma_N$} \end{array} \right. \\
...
\end{eqnarray*}
We will use the time derivative to numerically integrate $T_{XY}$, and the second derivative for parameter-fitting.
For later convenience define $F_{XY} \equiv \frac{\partial}{\partial t} T_{XY}$.
...
\begin{eqnarray*}
  ...
  \\
  \frac{\partial}{\partial \theta}F_{XY} & = &
...
\begin{eqnarray*}
  ...
  {\bf K}^{(n,1)} & = & {\bf F}\left(\Theta,{\bf T}^{(n)}\right) \\
  {\bf K}^{(n,2)} & = & {\bf F}\left(\Theta,{\bf T}^{(n)} + \frac{\Delta_n}{2} {\bf K}^{(n,1)}\right) \\
  {\bf K}^{(n,3)} & = & {\bf F}\left(\Theta,{\bf T}^{(n)} + \frac{\Delta_n}{2} {\bf K}^{(n,2)}\right) \\
  {\bf K}^{(n,4)} & = & {\bf F}\left(\Theta,{\bf T}^{(n)} + \Delta_n {\bf K}^{(n,3)}\right)
\end{eqnarray*}
