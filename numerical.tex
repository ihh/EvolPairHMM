...
$\tau \subseteq \Phi \times \gappedalphabet \times \gappedalphabet \times \Phi$ is the transition relation ($\epsilon$ is the empty string), and
...
define the Forward\footnote{So-called because it can be computed by the Forward algorithm, after removing any null transition cycles.} weight
...
\[
  ...
  \end{array} \right.
\]
Note this composition preserves alignment transitivity: characters that are aligned\footnote{In the sense of being associated with the same transition.}
through both $\mathbb{A}$ and $\mathbb{B}$ are aligned by $\mathbb{AB}$.
...
Given a conditionally-normalized\footnote{Meaning the Forward weight is nonnegative; and its sum over output sequences, for a given input sequence, is 1.}
