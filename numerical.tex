...
\textcolor{red}{Analyzing the $t\to\infty$ limit: consider separately the cases $\lambda<\mu$ (so $L \gg M$), $\lambda>\mu$ (so $L \ll M$), and $\lambda=\mu$ (so $L=M$). It looks like in each of these cases $q'<0$ at large $t$, so $q \to 0$. Then can consider the different cases for $b'$. Finally $a' \to 0$ but very slowly.}
