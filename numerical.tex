...
Given a Markov chain with match, insert, delete, and null states $\{ \sigma_M = \{ 1 \}, \sigma_I, \sigma_D, \sigma_N \}$
we seek the expected number of times $T_{XY}$ that the shortest closed walk from/to state 1,
...
Let ${\bf U}$ be the transition matrix of the Markov chain.
Then
...
\subsection{Rate of change $F_{XY}$ of expected transition count}
...
where $S_x = \sum_{y \in \{M,I,D\}} T_{xy}  = \sum_{y \in \{M,I,D\}} T_{yx}$
and noting that $S_M = 1$.
