...
Rewriting the ODEs with $A \equiv T_{MM}, B \equiv T_{MI}, F \equiv T_{IM}, Q \equiv T_{DI}$
yields
\begin{eqnarray*}
  \frac{dA}{dt} & = & \frac{\mu  (y-1) B F}{1 -y (1+B+Q)-(1-y) e^{\frac{\lambda  t}{1-x}}} - (\lambda +\mu ) A \\
  \frac{dB}{dt} & = & \frac{\mu  (B+Q) B}{1 -y (1+B+Q)-(1-y) e^{\frac{\lambda  t}{1-x}}} + \lambda (1-B) \\
  \frac{dF}{dt} & = & \frac{\mu  (B+Q) F}{1 -y (1+B+Q)-(1-y) e^{\frac{\lambda  t}{1-x}}} + \lambda A \\
  \frac{dQ}{dt} & = & \frac{\mu  (B+Q) \left(1-e^{\frac{\lambda  t}{1-x}}+Q\right)}{1 -y (1+B+Q)-(1-y) e^{\frac{\lambda  t}{1-x}}}
\end{eqnarray*}
...
Need to rewrite $c,g,h,p,r$ in terms of $a,b,f,q$ (and $\lambda,\mu,x,y,t$).

Then use Mathematica e.g.

{\tt FullSimplify[D[i2m[T]/SI, T] /. {i2m[T] -> f SI, i2m'[T] -> Di2m}] }
