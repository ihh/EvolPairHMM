...
For input and output sequences ${\cal X}, {\cal Y} \in \Omega^\ast$ define the Forward\footnote{So-called because it can be computed by the Forward algorithm, after removing any null cycles.} weight
...
Given a conditionally-normalized\footnote{Meaning the Forward weight is between 0 and 1, and summed over output sequences for a given input sequence is 1.}
