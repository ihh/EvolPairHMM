...
\subsubsection{Trying to rewrite and solve the ODEs}

Rewriting the ODEs with $A \equiv T_{MM}, B \equiv T_{MI}, F \equiv T_{IM}, Q \equiv T_{DI}, S \equiv S_I$
yields
\begin{eqnarray*}
  \frac{dA}{dt} & = & \frac{\mu  (y-1) B F}{y(S-B-Q) - S} - (\lambda +\mu ) A \\
  \frac{dB}{dt} & = & \frac{\mu  (B+Q) B}{y(S-B-Q) - S} + \lambda (1-B) \\
  \frac{dF}{dt} & = & \frac{\mu  (B+Q) F}{y(S-B-Q) - S} + \lambda A \\
  \frac{dQ}{dt} & = & \frac{\mu  (B+Q) (Q-S)}{y(S-B-Q) - S} \\
  S & = & \exp\left(\frac{\lambda t}{1-x}\right)-1
\end{eqnarray*}

but this is problematic because the common denominator is undefined at $t=0$.
Can we rewrite these in terms of $a,b,f,q$ and resolve this problem?
No, that wouldn't help because $a',b',f',q'$ will involve $A',B',F',Q'$ so the undefined quantities will remain.
...
Another idea: $y(S-B-Q)-S=yG-S$. Note that $\lim_{t \to 0} G(t) = S(t)-1$.

Can we set $S(t)$ equal to something plus $\epsilon$ and take the limit as $\epsilon \to 0$?

More broadly: these formulae for the derivatives may be undefined at $t=0$, but the derivatives themselves clearly are not, as can be seen by taking limits like this.
